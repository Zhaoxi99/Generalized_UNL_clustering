...
y=rep(0,n)
y[sign==0]=12*x[sign==0]+eps[sign==0]
y[sign==1]=-12*x[sign==1]+eps[sign==1]
...
for(i in 1:length(ygrid)){
  for(j in 1: length(xgrid)){
    true_dense_cate1[i,j]=mixAK::dMVNmixture(x=ygrid[i],weight = c(0.5,0.5),mean = c(12*xgrid[j],-12*xgrid[j]),Sigma = c(0.4^2,0.4^2))
  }
}
