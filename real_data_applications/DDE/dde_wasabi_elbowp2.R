...
###
ind1 <- which(output_WASABI$particles[2,] == 1)
ind2 <- which(output_WASABI$particles[2,] == 2)
ind3 <- which(output_WASABI$particles[2,] == 3)
...
theme_set(theme_bw())
ggplot(dde)+geom_point(aes(x=DDE,y=GAD,col=as.factor(output_WASABI$particles[2,])),alpha=.5, cex=.5)+
